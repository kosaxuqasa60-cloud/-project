...
export function NewQuestion() {
  ...
  const [openCats, setOpenCats] = useState<string[]>(["knowledge", "usage"])
          ...
          <section className="rounded-xl border border-border bg-card">
            <div className="border-b border-border p-4">
              ...
              <p className="mb-3 text-xs leading-relaxed text-muted-foreground">
                从受控词表中选择，或让 AI 依据题干自动打标，老师确认即可。
              </p>
              <button
                onClick={runAi}
                        ...
                        {cat.options.map((opt) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {opt}
                              {ai && (
                                <Sparkles className="size-3 text-brand" aria-label="AI 推荐" />
                              )}
                            </button>
                          )
                        })}
